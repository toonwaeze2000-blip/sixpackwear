...
async function register(p){
 ...
 if(r.data.user)showOtpStep({mode:'register',name:p.name,email:p.email});
}
async function login(p){
 const r=await sb.auth.signInWithPassword({email:p.email,password:p.password});
 if(r.error)throw r.error;
 msg('Signed in. Opening your shop…');setTimeout(()=>{window.location.href='/catalog.html?view=all'},350);
}
function showOtpStep(p){
 content.innerHTML=`<div class="authForm"><div class="kicker">Email verification</div><h4 style="margin:0;font-size:22px;text-transform:uppercase">Enter your OTP</h4><div class="authMessage">We sent a verification code to <strong>${esc(p.email)}</strong>. Enter all digits from the email.</div><label>Verification code<input id="otp" inputmode="numeric" autocomplete="one-time-code" maxlength="8" pattern="[0-9]{6,8}" placeholder="00000000" required></label><div class="authMessage" id="authMessage"></div><button class="authSubmit" id="verifyOtp">Verify account</button><button class="authSecondary" type="button" id="resendOtp">Resend code</button><button class="authSecondary" type="button" id="backAuth">Back</button></div>`;
 const otp=document.getElementById('otp');otp.focus();
 document.getElementById('verifyOtp').onclick=async()=>{const token=otp.value.trim();if(!/^\d{6,8}$/.test(token)){msg('Enter the complete verification code from your email.');return}const btn=document.getElementById('verifyOtp');btn.disabled=true;try{const r=await sb.auth.verifyOtp({email:p.email,token,type:'email'});if(r.error)throw r.error;msg('Account verified. Opening your shop…');setTimeout(()=>{window.location.href='/catalog.html?view=all'},700)}catch(err){btn.disabled=false;msg(err.message)}};
...
async function render(){const r=await sb.auth.getUser();if(r.error){form('login');return}if(r.data.user){content.innerHTML=`<div class="accountState"><div class="accountLine"><span>Email</span><strong>${esc(r.data.user.email||'')}</strong></div><div class="authMessage">You are signed in with your 6PACKWEAR account.</div><button class="authSubmit" id="shopBtn">Open shop</button><button class="authSecondary" id="logoutBtn">Log out</button></div>`;document.getElementById('shopBtn').onclick=()=>{window.location.href='/catalog.html?view=all'};document.getElementById('logoutBtn').onclick=async()=>{await sb.auth.signOut();form('login')}}else form('login')}
