...
async function login(p){
 ...
 if(r.error)throw r.error;
 msg('Signed in. Welcome back.');setTimeout(close,500);
}
function showOtpStep(p){
 ...
 document.getElementById('verifyOtp').onclick=async()=>{const token=otp.value.trim();if(!/^\d{6}$/.test(token)){msg('Enter the 6-digit code from your email.');return}const btn=document.getElementById('verifyOtp');btn.disabled=true;try{const r=await sb.auth.verifyOtp({email:p.email,token,type:'signup'});if(r.error)throw r.error;msg('Account verified. Welcome to 6PACKWEAR.');setTimeout(close,700)}catch(err){btn.disabled=false;msg(err.message)}};
