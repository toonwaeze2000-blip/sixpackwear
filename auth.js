...
async function startRegistration(name,email,password){const r=await sb.auth.signInWithOtp({email,options:{shouldCreateUser:true,data:{full_name:name}}});if(r.error)throw r.error;showOtpStep({name,email,password});}
...
async function forgotPassword(){
 ...
 try{const r=await sb.auth.resetPasswordForEmail(email,{redirectTo:window.location.origin});if(r.error)throw r.error;msg('Password reset instructions were sent to your email. Check your inbox.')}
 catch(err){msg(err.message)}
}
function showReset(){content.innerHTML=`<div class="authForm"><label>New password<input id="newPassword" type="password" minlength="8" autocomplete="new-password" required></label><label>Repeat password<input id="newPassword2" type="password" minlength="8" autocomplete="new-password" required></label><div class="authMessage" id="authMessage">Choose a new password for your account.</div><button class="authSubmit" id="savePassword">Save new password</button></div>`;document.getElementById('savePassword').onclick=async()=>{const a=document.getElementById('newPassword').value,b=document.getElementById('newPassword2').value;if(a.length<8){msg('Password must be at least 8 characters.');return}if(a!==b){msg('Passwords do not match.');return}try{const r=await sb.auth.updateUser({password:a});if(r.error)throw r.error;msg('Password changed successfully.');setTimeout(close,700)}catch(err){msg(err.message)}}}
...
sb.auth.onAuthStateChange((event)=>{if(event==='PASSWORD_RECOVERY'){modal.classList.add('show');modal.setAttribute('aria-hidden','false');showReset()}});
