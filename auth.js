...
function showOtpStep(p){
 content.innerHTML=`<div class="authForm"><div class="kicker">Email verification</div><h4 style="margin:0;font-size:22px;text-transform:uppercase">Enter your OTP</h4><div class="authMessage">We sent a 6-digit verification code to <strong>${esc(p.email)}</strong>. Check your inbox and spam folder.</div><label>Verification code<input id="otp" inputmode="numeric" autocomplete="one-time-code" maxlength="6" pattern="[0-9]{6}" placeholder="000000" required></label><div class="authMessage" id="authMessage"></div><button class="authSubmit" id="verifyOtp">Verify account</button><button class="authSecondary" type="button" id="resendOtp">Resend code</button><button class="authSecondary" type="button" id="backAuth">Back</button></div>`;
 const otp=document.getElementById('otp');otp.focus();
 document.getElementById('verifyOtp').onclick=async()=>{const token=otp.value.trim();if(!/^\d{6}$/.test(token)){msg('Enter the 6-digit code from your email.');return}const btn=document.getElementById('verifyOtp');btn.disabled=true;try{const r=await sb.auth.verifyOtp({email:p.email,token,type:'signup'});if(r.error)throw r.error;msg('Account verified. Opening your shop…');setTimeout(()=>{window.location.href='/catalog.html?view=all'},700)}catch(err){btn.disabled=false;msg(err.message)}};
